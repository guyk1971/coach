...
# for learning from pure random experience, heatup_steps=DATASET_SIZE
experience_generating_schedule_params.heatup_steps = EnvironmentSteps(DATASET_SIZE)

# learning from trained agent (not random), set the heatup_steps to be < DATASET_SIZE:
...
# NN configuration
experience_generating_agent_params.network_wrappers['main'].learning_rate = 0.001
experience_generating_agent_params.network_wrappers['main'].batch_size = 64
...
# experience_generating_agent_params.exploration.epsilon_schedule = LinearSchedule(1.0, 0.1, DATASET_SIZE//10)
...
#  Environment #
################
env_params = GymVectorEnvironment(level='LunarLander-v2')
